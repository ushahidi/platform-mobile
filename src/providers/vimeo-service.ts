...
export class VimeoService extends HttpService {
  ...
  public uploadVideo(file:string, title:string, description:string):Promise<string> {
    this.logger.info(this, "uploadVideo", file);
    return new Promise((resolve, reject) => {
      this.createTicket().then((ticket:any) => {
        ...
        this.uploadFile(uploadUrl, file).then((uploaded:any) => {
          this.logger.info(this, "uploadVideo", "uploadFile", uploaded);
          this.completeVideo(completeUrl).then((completed:any) => {
              ...
              if (location) {
                ...
                this.updateVideo(identifier, title, description).then((updated:any) => {
  ...
  protected createTicket():Promise<any> {
    return new Promise((resolve, reject) => {
      let url = "https://api.vimeo.com/me/videos";
      let params = { type: "streaming" };
      this.httpPost(url, this.accessToken, params).then((data:any) => {
  ...
  protected uploadFile(url:string, file:any):Promise<any> {
    return new Promise((resolve, reject) => {
      this.logger.info(this, "uploadFile", url, file);
      this.fileSize(file).then((fileSize) => {
        this.logger.info(this, "uploadFile", "fileSize", fileSize);
        let mimeType = this.mimeType(file);
        this.fileUpload(url, this.accessToken, file, null, "PUT", mimeType, this.acceptType, mimeType, fileSize).then((data:any) => {
  ...
  protected updateVideo(identifier:string, name:string=null, description:string=null):Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      this.httpPatch(url, this.accessToken, params).then((data:any) => {
  ...
  protected completeVideo(url:string):Promise<any> {
    return new Promise((resolve, reject) => {
      this.logger.info(this, "completeVideo", url);
      this.httpDelete(url, this.accessToken).then((data:any) => {
